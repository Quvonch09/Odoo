...
class OpFaculty(models.Model):
        ...
        for record in self:
            if not record.emp_id:
                vals = {
                    ...
                    'sex': record.gender,
                    # Qo'shimcha sinxronizatsiya: Phone, Partner
                    'work_phone': record.phone or False,
                }
                # Agar Odoo 19 da hr.employee ni to'g'ridan to'g'ri bog'lash zarur bo'lsa
                if hasattr(self.env['hr.employee'], 'address_home_id'):
                    vals['address_home_id'] = record.partner_id.id
    ...
    def create(self, vals_list):
        ...
        # Baza bitta qilinishi (sinxronizatsiya) uchun Yaratilganda o'zi avtomat employee yaratib qo'shib ketadi
    ...
    def write(self, vals):
        ...
        # O'zgarishlar kiritilganda HR Employee ham mos ravishda yangilanadi
        for record in self:
            if record.emp_id:
                emp_vals = {}
                if 'name' in vals and record.name:
                    emp_vals['name'] = record.name
                if 'nationality' in vals:
                    emp_vals['country_id'] = record.nationality.id if record.nationality else False
                if 'gender' in vals:
                    emp_vals['sex'] = record.gender
                if 'phone' in vals:
                    emp_vals['work_phone'] = record.phone or False
                
                if emp_vals:
                    record.emp_id.write(emp_vals)
        return res
